...
const URL = "/api/empleados-parcelas-cultivos";
...
describe.skip(`GET ${URL}`, () => {
  ...
  testFunctionGet(URL, "Debe retornar un json con los registros de empleados", 200, API, HEADERS);
...
describe.skip(`POST ${URL}`, () => {
  const nuevoEmpleadoParcelaCultivo = {
    id_parcela_cultivo: "1",
    id_usuario: "2",
  };

  testFunctionPost(URL, "Debe retornar un error al no enviar el token", nuevoEmpleadoParcelaCultivo, 401, API);

  const empleadoParcelaSinIdParcela = { ...nuevoEmpleadoParcelaCultivo };
  empleadoParcelaSinIdParcela.id_parcela_cultivo = "";

  testFunctionPost(URL, "Debe retornar un error al asignar un empleado a una parcela con un id de parcela inexistente en la bd o que este vacio", empleadoParcelaSinIdParcela, 400, API, HEADERS);

  const empleadoParcelaSinIdUsuario = { ...nuevoEmpleadoParcelaCultivo };
  empleadoParcelaSinIdUsuario.id_usuario = "";
  testFunctionPost(URL, "Debe retornar un error al asignar un empleado a una parcela con un id de usuario inexistente en la bd o que este vacio", empleadoParcelaSinIdUsuario, 400, API, HEADERS);

  testFunctionPost(URL, "Asignar un empleado a una parcela", nuevoEmpleadoParcelaCultivo, 201, API, HEADERS);
});

describe(`PUT ${URL}/:id`, () => {
  const actualizaEmpleadoParcelaCultivo = {
    id_parcela_cultivo: "1",
    id_usuario: "2",
  };

  testFunctionPut(`${URL}/1`, "Debe retornar un error al no enviar el token", actualizaEmpleadoParcelaCultivo, 401, API);

  testFunctionPut(`${URL}/5`, "Actualizar un almacen con un id inexistente", actualizaEmpleadoParcelaCultivo, 400, API, HEADERS);

  const empleadoParcelaSinIdParcela = { ...actualizaEmpleadoParcelaCultivo };
  empleadoParcelaSinIdParcela.id_parcela_cultivo = "";

  testFunctionPut(`${URL}/1`, "Debe retornar un error al asignar un empleado a una parcela con un id de parcela inexistente en la bd o que este vacio", empleadoParcelaSinIdParcela, 400, API, HEADERS);

  const empleadoParcelaSinIdUsuario = { ...actualizaEmpleadoParcelaCultivo };
  empleadoParcelaSinIdUsuario.id_usuario = "";
  testFunctionPut(`${URL}/1`, "Debe retornar un error al asignar un empleado a una parcela con un id de usuario inexistente en la bd o que este vacio", empleadoParcelaSinIdUsuario, 400, API, HEADERS);

  testFunctionPut(`${URL}/1`, "Actualizar un empleado parcela cultivo", actualizaEmpleadoParcelaCultivo, 200, API, HEADERS);
});
describe(`DELETE ${URL}/:id`, () => {
  testFunctionDelete(`${URL}/1`, "Debe retornar un error al no enviar el token", 401, API);

  testFunctionDelete(`${URL}/5`, "Eliminar un empleado parcela cultivo con un id inexistente", 400, API, HEADERS);

  testFunctionDelete(`${URL}/1`, "Eliminar un empleado parcela cultivo correctamente", 200, API, HEADERS);
});
